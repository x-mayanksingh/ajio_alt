...
const Footer: React.FC<FooterProps> = ({ isOnDressingScreen = false }) => {
  ...
  return (
    <footer className={`fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-gray-200/60 p-3 z-50 ${isOnDressingScreen ? 'hidden sm:block' : ''}`}>
      <div className="mx-auto flex flex-col sm:flex-row items-center justify-between text-xs text-gray-500 max-w-7xl px-4">
        <p>
          Created with{' '}
          <a 
            href="https://aistudio.google.com/app/project/b5b59743-5c8b-45e5-9469-8a892973f08f" 
            target="_blank" 
